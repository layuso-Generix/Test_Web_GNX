/* =========================================================
  example-viewer.js
  ---------------------------------------------------------
  Funciones compartidas para visualización de ejemplos
  JSON/XML/XSD.
...
function downloadExample(index, fileName) {
  downloadBlob(window._examples?.[index], fileName);
}
...
function toggleExampleCode(previewId, button) {
  const container = document.getElementById(previewId);
  if (!container) {return;}
  const open = container.style.display !== "none";
  container.style.display = open ? "none" : "block";
  button.textContent = open ? t("btn.viewContent") : t("btn.hide");
  const card = container.closest(".file-card");
  ...
  if (!open && !container.dataset.rendered) {
    const mini = container.querySelector(".ejemplo-mini");
    const index = mini ? Number(mini.dataset.idx) : 0;
    const name = mini ? mini.dataset.name : "FILE";
    const raw = window._examples?.[index];

    if (raw != null) {
      const header =
        `<span>${esc(name)}</span>` +
        `<button class="copy-btn" onclick="copyExample(${index}, this)">${t("btn.copy")}</button>`;
      renderJsonMinimap(container, raw, [], header);
...
window.toggleExampleCode = toggleExampleCode;
