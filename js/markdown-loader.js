...
function inlineMd(text) {
    return esc(text)
        ...
        .replace(
            /`(.+?)`/g,
            '<code style="background:var(--gray-100);padding:1px 5px;border-radius:3px;font-family:monospace">$1</code>',
        )
        .replace(
            /\[([^\]]+)\]\(([^)]+)\)/g,
            '<a class="download-link" href="$2" target="_blank">$1</a>',
        );
}
function simpleMarkdown(md, skipFirstH = false) {
    ...
    function closeCode() {
        if (inCode) {
            html += `<div class="code-panel"><pre class="code-pre">${esc(code.join("\n"))}</pre></div>`;
            inCode = false;
            code = [];
        }
    }
    for (const raw of lines) {
        ...
        if (line.startsWith("### ")) {
            ...
            if (skipFirstH && !first) {
                first = true;
                html += `<div style="border-bottom:2px solid var(--generix-orange-soft);margin:0 0 24px"></div>`;
            ...
            closeUl();
            html += `<div class="highlight-box">${inlineMd(line.slice(1).trim())}</div>`;
        } else if (line.trim() === "") {
            closeUl();
        } else {
            closeUl();
