      ...
      .map(
        (s) =>
          `<a class="card" href="#${esc(s.id)}">
            <div class="card-icon">${esc(s.icon || s.format.toUpperCase())}</div>
            <div class="card-meta">
              <span class="badge badge-${esc(s.format)}">${esc(s.format)}</span>
              <span class="badge badge-cat">${esc(s.group)}</span>
            </div>
            <h3>${esc(getLocalized(s.title))}</h3>
            <p>${esc(getLocalized(s.description))}</p>
            <span class="card-link">${t("file.view")}</span>
          </a>`,
      )
      .join("");
  },
